...
use openminedash_core::{
    AppState, CoinDefinition, CrashRecoveryState, MiningConfig, MiningStatus, PoolConfig, Profile,
...
#[tauri::command]
pub async fn discover_pools(symbol: String) -> Result<Vec<openminedash_pools::DiscoveredPool>, String> {
    Ok(openminedash_pools::discover_pools_with_fallback(&symbol).await)
}

// ============================================================================
// Pool Management Commands (Add/Remove pools from coins)
// ============================================================================

#[tauri::command]
pub async fn add_pool_to_coin(
    coin_id: String,
    pool: PoolConfig,
) -> Result<CoinDefinition, String> {
    openminedash_core::AppState::add_pool_to_coin(&coin_id, pool).map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn remove_pool_from_coin(
    coin_id: String,
    stratum_url: String,
) -> Result<CoinDefinition, String> {
    openminedash_core::AppState::remove_pool_from_coin(&coin_id, &stratum_url).map_err(|e| e.to_string())
}
